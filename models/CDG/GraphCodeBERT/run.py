...
logging.basicConfig(format='%(asctime)s - %(levelname)s - %(name)s -   %(message)s',
                    datefmt='%m/%d/%Y %H:%M:%S',
                    level=logging.INFO)
# from parser1 import DFG_python,DFG_java,DFG_ruby,DFG_go,DFG_php,DFG_javascript,DFG_csharp
from parser1 import DFG_java, DFG_python
...
def convert_examples_to_features(examples, tokenizer, args, stage=None):
    ...
    for example_index, example in enumerate(tqdm(examples, total=len(examples))):
        ##extract data flow
        code_tokens, dfg = extract_dataflow(example.source, parsers[args.source_lang], args.source_lang)
...
def main():
    ...
    if args.do_train:
        ...
        for epoch in range(args.num_train_epochs):
            ...
            if args.do_eval and epoch in [int(args.num_train_epochs * (i + 1) // 20) for i in range(20)]:
                print('Eval model with dev dataset')
                ...
                for batch in eval_dataloader:
                    ...
                    source_ids, source_mask, position_idx, att_mask, target_ids, target_mask = batch
                    with torch.no_grad():
                        preds, final_attn_output_weights = model(source_ids, source_mask, position_idx, att_mask)
                        for pred in preds:
                            t = pred[0].cpu().numpy()
                            t = list(t)
                            if 0 in t:
    ...
    if args.do_test:
        ...
        for idx, file in enumerate(files):
            ...
            for prediction, attention, eval_example, eval_feature, encoded_output in zip(p, attentions, eval_examples,
                                                                                         eval_features,
                                                                                         encoded_outputs):
                pickle_entry = dict()
